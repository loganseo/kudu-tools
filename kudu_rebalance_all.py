...
json_tablet_status = kmod.extract_dist_status(masters, table_name, target_partition)
        ...
        while candidate_queue.qsize():
            ...
            if move_cnt != 0 \
                ...
                if check_cnt == 0 and len(moved_list) > 0:
                    kmod.remove_replica(masters, table_name, moved_list, check_cnt)
                # moving_replica_list 초기화
                moving_replica_list = []
                moved_list = []
                move_cnt = 0
        print("\nKudu Rebalance 완료 (소요시간: %s)" % kmod.calc_elapse_time(start_time))
        sys.stdout.flush()
    else:
